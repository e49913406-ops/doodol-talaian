...
TOKEN = "8744016286:AAECgqXRDn2Cy07agOUM-NJz
WOgtk38LKB38"
...
if text == "سطح شهر":  
    ...
    for user_id, p in ranking:  
        ...
        if level == 1:  
            level_text = "سطح یک در شهر"  
        elif level == 2:  
            level_text = "سطح دو در شهر"  
        elif level == 3:  
            level_text = "سطح سه در شهر"  
        elif level == 4:  
            level_text = "سطح چهار در شهر"  
        elif level == 5:  
            level_text = "سطح پنج در شهر"  
        else:  
            level_text = f"سطح {level} در شهر"  

        lines.append(  
            f'{mention_user(user_id, p["name"])} {level}/30 ({level_text})'  
        )  
...
سطح الانت:: 30/{new_level}"""
)
else:
await update.message.reply_text(
"""دودول ات طلایی شد😼
یادت نره با هر پنج بار دودول طلا گفتن یک سطح میری بالا تر!"""
)
